...
func Create(cfg *core.Config) *Application {
...
func (a *Application) Dispatcher() *core.LocalDispatcher {
	return a.Disp
}
	...
	for _, mod := range modules {
		...
		for _, cap := range mod.Capabilities() {
			for _, mr := range cap.Messages {
				...
				if err := a.Disp.RegisterHandler(mr.Name, mr.Handler, core.HandlerInfo{
					Name:        mr.Name,
				...
				if mr.Input.Schema != nil {
					if issues, ok := core.ValidateInputSchema(mr.Input.Schema); !ok && len(issues) > 0 {
...
func (a *Application) Reset(cfg *core.Config, version string) {
	_ = a.Loggers.File.Sync()
	_ = a.PersistenceManager.Close()
	_ = a.Loggers.Close()
	...
	mod := app.New(cfg, a.Dispatcher(), app.Options{
