...
func NewUpdateUserHandler(users *UserModel) core.MessageHandler {
	return func(ctx context.Context, msg core.Message) (*registration.Result, error) {
		doc := msg.Input()
		userID, _ := doc.GetOr("arguments.user_id", "").(string)
...
func NewChangePasswordHandler(users *UserModel) core.MessageHandler {
	return func(ctx context.Context, msg core.Message) (*registration.Result, error) {
		doc := msg.Input()
		userID, _ := doc.GetOr("arguments.user_id", "").(string)
		body, _ := doc.GetOr("payload", nil).(map[string]any)
		...
		d, err := users.GetByID(ctx, userID)
		if err != nil {
			return nil, core.ErrNotFound.WithOperation("change_password")
		}

		if users.IsDeleted(d) {
			return nil, core.ErrUserDeleted.WithOperation("change_password")
		}
		...
		if !core.CheckPassword(currentPassword, storedPassword) {
			return nil, core.ErrInvalidCredentials.WithOperation("change_password")
		}
...
func NewDeleteUserHandler(users *UserModel) core.MessageHandler {
	return func(ctx context.Context, msg core.Message) (*registration.Result, error) {
		doc := msg.Input()
		userID, _ := doc.GetOr("arguments.user_id", "").(string)
...
func NewUserCreateDocumentHandler(users *UserModel) core.MessageHandler {
	return func(ctx context.Context, msg core.Message) (*registration.Result, error) {
		doc := msg.Input()
...
func NewUserUpdateDocumentHandler(users *UserModel) core.MessageHandler {
	return func(ctx context.Context, msg core.Message) (*registration.Result, error) {
		doc := msg.Input()
