...
func Registrations(deps Dependencies) []abstract.MessageRegistration {
	return []abstract.MessageRegistration{
		...
		{Name: "system:policies:rule:validate", Handler: NewValidateRuleHandler(deps.LiveRules), Description: "Validate a CEL rule expression", Enabled: true, Intent: registration.Check, Input: core.Input{Schema: policyValidateInputSchema(), Payload: definition.FieldTypeObject}, Output: policyValidateOutputSchema()},
