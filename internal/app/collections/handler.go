...
func NewDocumentDeleteHandler(persist persistence.Persistence) core.MessageHandler {
	return func(ctx context.Context, msg core.Message) (*registration.Result, error) {
		doc := msg.Input()
		name, _ := doc.GetOr("arguments.name", "").(string)
...
func NewDocumentUpdateHandler(persist persistence.Persistence) core.MessageHandler {
	return func(ctx context.Context, msg core.Message) (*registration.Result, error) {
		doc := msg.Input()
		name, _ := doc.GetOr("arguments.name", "").(string)
...
func RegisterDocumentHandlers(r core.Registry, persist persistence.Persistence, name string) error {
	...
	docCmdInfo := core.HandlerInfo{Description: fmt.Sprintf("Create a document in %q", name), Enabled: true}
	docQryInfo := core.HandlerInfo{Description: fmt.Sprintf("Get a document from %q", name), Enabled: true}
	docUpdInfo := core.HandlerInfo{Description: fmt.Sprintf("Update a document in %q", name), Enabled: true}
	docDelInfo := core.HandlerInfo{Description: fmt.Sprintf("Delete a document from %q", name), Enabled: true}
