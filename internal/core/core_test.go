...
func (m *mockPersister) Insert(_ context.Context, entry AccessLogEntry) error {
	m.entries = append(m.entries, entry)
	return nil
}

func TestAccessLogDispatcher(t *testing.T) {
	...
	disp := NewAccessLogDispatcher(next, persister)
	...
	if persister.entries[0].MessageName != "test:cmd" {
		t.Fatalf("expected MessageName test:cmd, got %q", persister.entries[0].MessageName)
	}
	if persister.entries[0].Status != AccessStatusOK {
		t.Fatalf("expected Status OK, got %q", persister.entries[0].Status)
	}
...
func TestAccessLogDispatcherError(t *testing.T) {
	persister := &mockPersister{}
	next := NewLocalDispatcher()
	handler := func(ctx context.Context, msg Message) (*registration.Result, error) {
	...
	ctx := ContextWithAccessLogIdentity(context.Background(), "u1", "tok1")
	ctx = ContextWithTransportMetadata(ctx, "10.0.0.1", "curl", "GET", "/test", "req-1")
	...
	if persister.entries[0].Status != AccessStatusError {
		t.Fatalf("expected Status Error, got %q", persister.entries[0].Status)
	}
	if persister.entries[0].UserID != "u1" {
		t.Fatalf("expected UserID u1, got %q", persister.entries[0].UserID)
	}
	if persister.entries[0].Credential != "tok1" {
		t.Fatalf("expected Credential tok1, got %q", persister.entries[0].Credential)
	}
	if persister.entries[0].RequestID != "req-1" {
		t.Fatalf("expected RequestID req-1, got %q", persister.entries[0].RequestID)
	}
	trans, ok := persister.entries[0].TransportMetadata.(map[string]any)
	if !ok {
		t.Fatal("expected TransportMetadata to be map")
	}
	if trans["method"] != "GET" {
		t.Fatalf("expected method GET, got %v", trans["method"])
	}
	if trans["client_ip"] != "10.0.0.1" {
		t.Fatalf("expected client_ip 10.0.0.1, got %v", trans["client_ip"])
	}
}
...
func TestContextWithAccessLogIdentity(t *testing.T) {
	ctx := ContextWithAccessLogIdentity(context.Background(), "user1", "cred1")

	userID, ok := ctx.Value(AccessLogUserIDKey).(string)
	if !ok {
		t.Fatal("expected string userID in context")
	}
	if userID != "user1" {
		t.Fatalf("expected userID user1, got %q", userID)
	}

	cred, ok := ctx.Value(AccessLogCredentialKey).(string)
	if !ok {
		t.Fatal("expected string credential in context")
	}
	if cred != "cred1" {
		t.Fatalf("expected credential cred1, got %q", cred)
	}
}

func TestContextWithAccessLogIdentityEmpty(t *testing.T) {
	ctx := ContextWithAccessLogIdentity(context.Background(), "", "")

	userID, _ := ctx.Value(AccessLogUserIDKey).(string)
	if userID != "" {
		t.Fatalf("expected empty userID, got %q", userID)
	}

	cred, _ := ctx.Value(AccessLogCredentialKey).(string)
	if cred != "" {
		t.Fatalf("expected empty credential, got %q", cred)
	}
}
