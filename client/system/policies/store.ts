...
> {
  ...
  async query(qdsl: QueryDSL<Policy>): Promise<Page<Policy>> {
    ...
    const policies: Document<Policy>[] = items;
    return {
      data: policies,
      loading: false,
      page: pagination ?? {
  ...
  async read(id: string): Promise<Document<Policy> | undefined> {
    try {
      const res = await this.client.dispatch<{ data: Document<Policy> }>(
        "system:policies:policy:list",
      );
      const items = (res.data?.data as any)?.policies ?? [];
      const match = items.find(
        (p: any) => p._id_ === id || p.operation === id || p.id === id,
      );
  ...
  async update(props: {
    ...
    if (props.data.rateLimit !== undefined)
      payload.rateLimit = props.data.rateLimit;
    if (props.data.throttle !== undefined)
      payload.throttle = props.data.throttle;
    const res = await this.client.dispatch<{ data: Document<Policy> }>(
