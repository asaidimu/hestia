...
func init() {
	GenerateCmd.Flags().BoolVar(&selfMode, "self", false, "Generate feature wiring (for the hestia library itself)")
}
...
func modPkg(suffix string) string {
...
func genModuleRegistry() {
	...
	if len(modules) == 0 {
		outPath := filepath.Join(rootDir, "internal", "autogen", "modules.go")
		os.Remove(outPath)
		return
	}
	...
	outDir := filepath.Join(rootDir, "internal", "autogen")
	os.MkdirAll(outDir, 0755)
