...
func Registrations(deps Dependencies) []abstract.MessageRegistration {
	return []abstract.MessageRegistration{
		{Name: "system:auth:session:create", Handler: NewCreateSessionHandler(deps.UserModel, deps.CredentialsProvider, deps.SessionTTL), Description: "Authenticate and receive a session token", Enabled: true, Intent: registration.Create, BootstrapSafe: true, Input: runtime.Input{Schema: loginInputSchema(), Payload: definition.FieldTypeObject}, Output: loginOutputSchema()},
		{Name: "system:auth:user:register", Handler: NewRegisterHandler(deps.UserModel), Description: "Register a new user", Enabled: true, Intent: registration.Create, Input: runtime.Input{Schema: registerInputSchema(), Payload: definition.FieldTypeObject}, Output: userOutputSchema()},
		{Name: "system:auth:session:delete", Handler: NewDeleteSessionHandler(), Description: "Logout", Enabled: true, Intent: registration.Delete, BootstrapSafe: true, Input: runtime.Input{Payload: definition.FieldTypeObject}},
		{Name: "system:auth:password:reset", Handler: NewPasswordResetHandler(deps.UserModel, deps.CredentialsProvider), Description: "Request password reset email", Enabled: true, Intent: registration.Create, Input: runtime.Input{Schema: passwordResetInputSchema(), Payload: definition.FieldTypeObject}, Output: messageOutputSchema()},
