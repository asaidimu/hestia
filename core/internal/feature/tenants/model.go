...
func (m *TenantModel) Create(ctx context.Context, name, domain string, metadata map[string]any) (*data.Document, error) {
...
func (m *TenantModel) GetByID(ctx context.Context, id string) (*data.Document, error) {
	col, err := m.collection(ctx)
	if err != nil {
		return nil, fmt.Errorf("access tenant collection: %w", err)
	}
...
func (m *TenantModel) GetByDomain(ctx context.Context, domain string) (*data.Document, error) {
	col, err := m.collection(ctx)
	if err != nil {
		return nil, fmt.Errorf("access tenant collection: %w", err)
	}
