...
// NewSystemUsers creates a fresh SystemUsers model bound to the given
// persistence. A fresh instance is required per module setup (and per test)
// because the generated InitSystemUsersModel singleton pins the first
// persistence it was built with, which breaks hestia's Reset lifecycle and
// isolated test persistence.
func NewSystemUsers(p base.Persistence, logger *zap.Logger, opts ...collection.ModelCollectionOptions[*SystemUser]) (*SystemUsers, error) {
...
// GetID returns the user's document ID.
func (u *SystemUser) GetID() string { return u.ID }
