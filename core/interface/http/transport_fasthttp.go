...
func (t *HTTPTransport) Handle(pattern string, handler abstract.Handler, opts ...abstract.RouteOption) {
        ...
        t.router.insert(method, path, routeEntry{handler: handler, streamingBody: ro.StreamingBody})
}
...
func (t *HTTPTransport) serveHTTP(ctx *fasthttp.RequestCtx) {
        ...
        if ok {
                ...
                // small default without ever reading the body. Streaming-body
                // routes skip the small default — the point of a streamed
                // import is a long-lived body — and fall back to the global
                // ceiling, which fasthttp enforces incrementally as the handler
                // reads.
                limit := int64(defaultBodyLimit)
                if entry.streamingBody {
                ...
                if entry.streamingBody {
                        // Streaming route: hand over the raw body reader. Calling
                        // ctx.Request.Body() here would force fasthttp to buffer
                        // the whole body first, defeating the point.
