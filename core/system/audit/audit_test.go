...
func queryHandler(p base.Persistence) abstract.MessageHandler {
	return collections.NewNamedCollectionQueryHandler("_audit_log_", p)
}
...
func TestAuditInsert(t *testing.T) {
	...
	if handler == nil {
		t.Fatal("system:collections:audit_log:query handler not found")
	}

	msg := testMessage{ctx: ctx, input: testutil.InputDoc(t, auditmodel.LogQueryInputSchema(), `{}`)}
...
func TestAuditQueryHandlerWithLimit(t *testing.T) {
	...
	input := testutil.InputDoc(t, auditmodel.LogQueryInputSchema(), `{
		"payload": {
			"pagination": {
				"type":          "offset",
				"offset":        0,
