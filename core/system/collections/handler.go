...
func NewCollectionCreateHandler(persist persistence.Persistence, policyOp abstract.BindingPolicyStore, registry abstract.Registry, logger *zap.Logger) abstract.MessageHandler {
	return func(ctx context.Context, msg abstract.Message) (*abstract.Result, error) {
		...
		return dispatch.NewDocumentResultFrom(&CollectionMetaView{
			Name:    name,
			Schema:  metadata.Schema.AsMap(),
			Created: now,
			Updated: now,
		})
	}
...
func NewCollectionDeleteHandler(persist persistence.Persistence, policyOp abstract.BindingPolicyStore, registry abstract.Registry, logger *zap.Logger) abstract.MessageHandler {
	return func(ctx context.Context, msg abstract.Message) (*abstract.Result, error) {
		...
		return dispatch.NewDocumentResultFrom(&CollectionDeletedView{Name: name})
	}
}
...
func NewDocumentCreateHandler(persist persistence.Persistence) abstract.MessageHandler {
	return func(ctx context.Context, msg abstract.Message) (*abstract.Result, error) {
		...
		r, ok := created.Data.(*document.Document)
		if !ok {
			return nil, fmt.Errorf("persistence returned %T, want *document.Document", created.Data)
		}
