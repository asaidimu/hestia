...
func (m *NotificationModel) filterExpired(docs []*document.Document) []*document.Document {
...
func (m *NotificationModel) List(ctx context.Context, userID, tenantID string, limit, offset int) ([]*document.Document, error) {
	...
	docs, err := documentsFrom(result.Data)
	if err != nil {
		return nil, err
	}
	return m.filterExpired(docs), nil
}
...
func (m *NotificationModel) CountUnread(ctx context.Context, userID, tenantID string) (int, error) {
	...
	return len(m.filterExpired(docs)), nil
}

// documentsFrom narrows a persistence-layer document set to its concrete
// *document.Document elements, which is what anansi collections always return.
func documentsFrom(docs data.DocumentSet) ([]*document.Document, error) {
