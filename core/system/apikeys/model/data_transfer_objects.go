...
// APIKeyListInput binds the dispatch input document's arguments for listing
// keys. It carries no fields: the acting user comes from the request claims
// (the list registration is claims-only), so no route argument is derived.
