...
func newTestMsg(t *testing.T, ctx context.Context, payload map[string]any) testMessage {
	t.Helper()
	j, err := json.Marshal(map[string]any{"payload": payload})
	if err != nil {
		t.Fatalf("marshal payload: %v", err)
	}
	return testMessage{
		ctx:   ctx,
		input: testutil.InputDoc(t, policies.PolicyValidateInputSchema(), string(j)),
	}
}
...
func (m testMsg) Input() data.Documenter { return m.input }
...
func TestValidateRuleHandler(t *testing.T) {
	...
	t.Run("simple CEL passes", func(t *testing.T) {
		...
		msg := newTestMsg(t, ctx, map[string]any{
