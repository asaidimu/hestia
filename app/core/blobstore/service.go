...
func (h *nsHandle) UpdateMetadata(ctx context.Context, key, contentType string, custom map[string]string) (*core.BlobMeta, error) {
	info, err := h.ns.UpdateMetadata(ctx, key, contentType, custom)
	if err != nil {
		return nil, err
	}
	return blobMetaFromInfo(info), nil
